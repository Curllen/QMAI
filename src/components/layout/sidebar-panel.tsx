...
import {
  ...
  makeSafeFileSlug,
} from "@/lib/wiki-filename";
import { useImportProgressStore } from "@/stores/import-progress-store";
import { openExternalUrl } from "@/lib/open-external-url";
import type { ReferenceToken } from "@/lib/reference/types";

const USAGE_GUIDE_URL =
  "https://tcnk9ik08e1c.feishu.cn/wiki/FWiSwYQKoifpwBk6mSRcSlB8nrh?from=from_copylink";
...
function shouldReadDismantlingOriginalFile(preprocessedText: string): boolean {
  return (
    preprocessedText.trim().toLowerCase() ===
    DISMANTLING_NO_PREPROCESSING_NEEDED
  );
}
...
export function DismantlingSidebarPanel() {
  ...
  const selectedDismantlingProjectId = useWikiStore(
    (s) => s.selectedDismantlingProjectId,
  );
  const setSelectedDismantlingProjectId = useWikiStore(
    (s) => s.setSelectedDismantlingProjectId,
  );
  const bumpDataVersion = useWikiStore((s) => s.bumpDataVersion);
  const [library, setLibrary] = useState<DismantlingLibrary>({
    version: 1,
    projects: [],
  ...
  useEffect(() => {
    ...
    void loadDismantlingLibrary(project.path)
      .then((value) => {
        ...
        const selected =
          ...
          value.projects[0] ??
          null;
        if (selected?.id !== selectedDismantlingProjectId)
          setSelectedDismantlingProjectId(selected?.id ?? null);
      })
      .finally(() => {
    ...
    setSelectedDismantlingProjectId,
  ]);

  async function importDismantlingCandidates(
    candidates: ChapterImportCandidate[],
    ...
    try {
      const normalizedTitle = normalizeDismantlingProjectTitle(
        titleFallback || "未命名拆文作品",
      );
      const existingProject = library.projects.find(
        (item) =>
          normalizeDismantlingProjectTitle(item.title) === normalizedTitle,
      );
      if (existingProject) {
        setSelectedDismantlingProjectId(existingProject.id);
        setImportStatus(`已存在相同拆文作品：${existingProject.title}`);
        window.alert(
          `已存在相同拆文作品：${existingProject.title}，不会重复显示。`,
        );
        return;
      }
      ...
      for (const candidate of sorted) {
        ...
        if (split.length <= 1) {
          ...
          chapters.push({
            ...
            title:
              split[0]?.title === "第1章"
                ? getFileStem(candidate.name) || `第${chapterNumber}章`
                : (split[0]?.title ?? getFileStem(candidate.name)),
  ...
  async function handleImportDismantlingFiles() {
    ...
    const selected = await open({
      ...
      title: "导入拆文文件",
      filters: [
        { name: "文档", extensions: ["txt", "md", "mdx", "doc", "docx"] },
      ],
    });
    ...
    await importDismantlingCandidates(
      candidates,
      getFileStem(candidates[0]?.name ?? "") || "拆文作品",
    );
  }
  ...
  async function handleDeleteDismantlingProject(item: DismantlingProject) {
    ...
    const confirmed = window.confirm(
      `确认删除拆文作品“${item.title}”吗？删除后会移除该作品的章节、拆文结果和结构记忆。`,
    );
    if (!confirmed) return;

    const nextProjects = library.projects.filter(
      (projectItem) => projectItem.id !== item.id,
    );
    const nextSelectedProjectId =
      selectedDismantlingProjectId === item.id
    ...
    setLibrary(normalizedLibrary);
    setSelectedDismantlingProjectId(
      normalizedLibrary.selectedProjectId ?? null,
    );
      ...
      <div className="flex shrink-0 items-center justify-between border-b px-3 py-2">
        ...
        <div className="flex items-center gap-1">
          <Button
            ...
            variant="ghost"
            className="h-8 w-8"
            onClick={() => void handleImportDismantlingFiles()}
            disabled={importing}
            title="导入文件"
          >
          ...
          <Button
            ...
            variant="ghost"
            className="h-8 w-8"
            onClick={() => void handleImportDismantlingFolder()}
            disabled={importing}
            title="导入文件夹"
          >
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-1 block text-xs text-muted-foreground">
                    {item.chapters.length} 章 · {item.structureMemory.length}{" "}
                    条结构记忆
                  </span>
                </span>
              </button>
...
function StorySimulationSidebarPanel() {
  ...
  const saveInterruptedState = useStorySimulationStore(
    (s) => s.saveInterruptedState,
  );
  const restoreInterruptedState = useStorySimulationStore(
    (s) => s.restoreInterruptedState,
  );
  const [deletingId, setDeletingId] = useState<string | null>(null);

  // 加载指定框架的历史推演结果
  const loadResultsForFramework = useCallback(
  ...
  const handleSelectFramework = (framework: StoryFramework) => {
    ...
    const hasInterruptedState =
      state.interruptedSimStates[framework.id] !== undefined;

    if (hasInterruptedState) {
      // 恢复该框架的中断状态
      restoreInterruptedState(framework.id);
  ...
  const handleSelectResult = (resultId: string) => {
    ...
    if (result) {
      ...
      if (result.agentSnapshot) {
        try {
          const { agents } = deserializeSimulationSnapshot(
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <span className="block truncate text-[11px] text-muted-foreground">
                    {result.report.recommendation?.slice(0, 25) ||
                      "查看推演结果"}
                    ...
                  </span>
                </div>
...
function countMemoryFileEntries(
...
function MemoryCenterListButton({
  ...
  return (
    ...
    >
      ...
      <span className="ml-3 shrink-0 text-sm font-semibold leading-none">
        {count}
      </span>
    </Button>
  );
...
async function getExistingChapterTitles(
  projectPath: string,
): Promise<Set<string>> {
...
async function getUniqueWikiPagePath(
  dir: string,
  fileName: string,
): Promise<string> {
...
export function SidebarPanel() {
  ...
  const selectedMemoryCenterEntry = useWikiStore(
    (s) => s.selectedMemoryCenterEntry,
  );
  const setSelectedMemoryCenterEntry = useWikiStore(
    (s) => s.setSelectedMemoryCenterEntry,
  );
  ...
  const [pendingCreate, setPendingCreate] =
    useState<KnowledgeCreateRequest | null>(null);
  ...
  const [memoryDecisionRequest, setMemoryDecisionRequest] =
    useState<ImportMemoryDecisionRequest | null>(null);
  ...
  const memoryDecisionResolveRef = useRef<
    ((decision: ImportMemoryDecision) => void) | null
  >(null);
  ...
  useEffect(() => {
    ...
    if (
      isChapter &&
      (pendingCreate.kind === "outline" || pendingCreate.kind === "folder")
    ) {
      cancelPendingCreate();
    }
  ...
  function confirmImportMemoryExtraction(
    kind: "chapter" | "outline",
    count: number,
  ): Promise<ImportMemoryDecision> {
  ...
  function confirmChapterMemoryExtraction(
    count: number,
  ): Promise<ImportMemoryDecision> {
    return confirmImportMemoryExtraction("chapter", count);
  }

  function confirmOutlineMemoryExtraction(
    count: number,
  ): Promise<ImportMemoryDecision> {
  ...
  async function extractImportedChapterMemories(
    projectPath: string,
    ...
    const titleByPath = new Map(
      importedChapters.map((chapter) => [chapter.path, chapter.title]),
    );
    ...
    const result = await runImportedChapterMemoryExtraction({
      ...
      onProgress: (progress) => {
        useImportProgressStore.getState().updateTask(taskId, {
          ...
          currentTitle: progress.currentPath
            ? (titleByPath.get(progress.currentPath) ?? progress.currentPath)
            : "",
        });
      },
    });
    ...
    useImportProgressStore
      .getState()
      .finishTask(taskId, result.cancelled ? "cancelled" : "done", {
  ...
  async function extractImportedOutlineMemories(
    projectPath: string,
    ...
    useImportProgressStore
      .getState()
      .finishTask(taskId, failed > 0 ? "error" : "done", {
        completed,
        total: importedPaths.length,
        currentTitle: "",
        message:
  ...
  async function finishChapterImport(
    projectPath: string,
    importedChapters: ImportedChapter[],
  ...
  async function handleImportChapterFiles() {
    ...
    const selected = await open({
      ...
      title: "导入章节文件",
      filters: [
        { name: "章节文档", extensions: [...CHAPTER_IMPORT_EXTENSIONS] },
      ],
    });
    if (!selected || (Array.isArray(selected) && selected.length === 0)) return;
    const sourcePaths = Array.isArray(selected) ? selected : [selected];

    const memoryDecision = await confirmChapterMemoryExtraction(
      sourcePaths.length,
    );
    ...
    try {
      ...
      const importedChapters = await importChapterFiles(
        projectPath,
        sourcePaths,
        {
  ...
  async function handleImportChapterFolder() {
    ...
    const candidates =
      await collectChapterImportCandidatesFromFolder(selectedFolder);
  ...
  async function handleImportOutlineFiles() {
    ...
    const selected = await open({
      multiple: true,
      title: t("novel.outlineImport.importFilesTitle", {
        defaultValue: "导入大纲文件",
      }),
      filters: [
        {
          name: t("novel.outlineImport.documentFilter", {
            defaultValue: "文档",
          }),
    ...
    try {
      ...
      if (importedPaths.length === 0) {
        window.alert(
          t("novel.outlineImport.emptyResult", {
            defaultValue: "没有找到可导入的大纲文档。",
          }),
        );
        return;
      }
      ...
      window.alert(
        t("novel.outlineImport.importFailed", {
          message,
  ...
  async function handleImportOutlineFolder() {
    ...
    const selected = await open({
      directory: true,
      title: t("novel.outlineImport.importFolderTitle", {
        defaultValue: "导入大纲文件夹",
      }),
    });
    ...
    const memoryDecision = await confirmOutlineMemoryExtraction(
      candidates.length,
    );
    if (memoryDecision === "cancel") return;
    const extractMemory = memoryDecision === "extract";
    ...
    try {
      const projectPath = normalizePath(project.path);
      const importedPaths = await importOutlineCandidates(
        projectPath,
        candidates,
      );
      if (importedPaths.length === 0) {
  ...
  async function handleCreateNextChapter(parentDir?: string) {
    ...
    try {
      ...
      const filePath = await getUniqueWikiPagePath(
        targetDir,
        makeChapterFileName(title, nextNumber),
      );
      const content = [
        "---",
        "type: chapter",
  ...
  const handleSendChapterToChat = useCallback(
  ...
  if (activeView === "lint" && novelMode) {
    const fileMap = new Map(
      memoryData?.files.map((file) => [file.key, file]) ?? [],
    );
    const entries = Object.keys(MEMORY_LABEL_KEYS)
      .filter((key) => key !== "dismantling-library")
      .map((key) => {
        if (key === "snapshots") {
          return {
            key,
      ...
      <div className="flex h-full flex-col">
        <div className="flex shrink-0 items-center justify-between border-b px-3 py-2">
          <div className="flex items-center gap-1.5 text-sm font-semibold">
            <PanelHeaderWithHelp
              title={t("novel.memoryCenter.title")}
              helpKey="memory"
            />
          </div>
          <Button
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex shrink-0 items-center justify-between border-b px-3 py-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            title={
              isChapter ? t("sidebar.newChapter") : t("sidebar.newOutline")
            }
      ...
      <RawSourcesSection
        onCancelExtraction={handleCancelImportMemoryExtraction}
      />
      <Dialog
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            <Button
              type="button"
              variant="outline"
              onClick={() => closeMemoryDecision("cancel")}
            >
              取消导入
            </Button>
            <Button
              type="button"
              variant="secondary"
              onClick={() => closeMemoryDecision("import-only")}
            >
              只导入
            </Button>
            <Button
              type="button"
              onClick={() => closeMemoryDecision("extract")}
            >
              提取记忆
            </Button>
          </DialogFooter>
