...
export async function runDeepChapterGeneration(
  ...
  const contextRequest = [
    ...
    planBlueprint ? `用户已确认的章节计划：\n${planBlueprint}` : "",
  ].filter(Boolean).join("\n\n");
  const workflowBaseParams = {
    mode: workflowProfile.mode,
    chapterNumber: input.chapterNumber ?? null,
  };
  const planExecutionSummary = buildChapterPlanExecutionSummary(planBlueprint ?? "");
  let executionContract: ChapterExecutionContract | null =
    resumeCheckpoint?.executionContract ?? null;
  if (executionContract && !hasUsableChapterExecutionContract(executionContract)) {
  ...
  if (!executionContract && workflowProfile.runExecutionContractBuild && planBlueprint) {
    try {
      ...
      executionContract = fallbackParseChapterExecutionContract(planBlueprint);
    }
  }
  ...
  const createCheckpoint = (
    stage: DeepChapterGenerationResumeStage,
    data: Partial<DeepChapterGenerationResumeCheckpoint> = {},
  ) => createResumeCheckpoint(input, stage, {
    planBlueprint,
  ...
  if (!resumeCheckpoint) {
    ...
    callbacks.onCheckpoint?.(createCheckpoint("after_context"));
    completeChapterWorkflowStep(
      callbacks,
      contextWorkflowStep,
      "上下文读取完成。",
  ...
  if (!taskBrief) {
    ...
    callbacks.onCheckpoint?.(
      createCheckpoint("after_task_brief", { taskBrief }),
    );
  }
  ...
  if (!draftContent) {
    ...
    callbacks.onCheckpoint?.(
      createCheckpoint("after_draft", { taskBrief, draftContent }),
    );
  }
  ...
  if (!workflowProfile.runPostDraftPlanAudits) {
    ...
    callbacks.onThinking?.(
      formatStageThinking(
        isFastMode ? "阶段4：快速完成" : "阶段4：标准完成",
        isFastMode
          ? "快速模式已完成任务书与正文初稿生成，直接采用正文初稿作为最终正文。"
          : "标准模式已完成任务书与正文初稿生成，直接采用正文初稿作为最终正文。",
      ),
    );
    emitDeepChapterActivity(callbacks, {
  ...
  if (!hasCheckpointReview(resumeCheckpoint)) {
    if (!shouldRunAiReview) {
      ...
      try {
        ...
        reviewFailureMessage = `AI 审稿失败：${getErrorMessage(err)}。已跳过自动返修，请在保存前手动复核正文。`;
        errorChapterWorkflowStep(callbacks, reviewWorkflowStep, err);
        callbacks.onThinking?.(
          formatStageThinking("阶段4：AI审稿", reviewFailureMessage),
        );
        emitDeepChapterActivity(callbacks, {
          id: `deep_chapter:validate_revision:error:${Date.now()}`,
          stageId: "validate_revision",
          kind: "analysis",
          title: "审稿失败",
          content: reviewFailureMessage,
        });
        reviewResults = [];
      }
      reviewResults = reviewResults || [];
      assertNotAborted(signal);
      if (!reviewFailureMessage) {
        completeChapterWorkflowStep(
          callbacks,
          reviewWorkflowStep,
  ...
  if (hasCheckpointRevision(resumeCheckpoint)) {
    ...
    completeChapterWorkflowStep(
      ...
      "AI 审稿失败，无法安全判断返修项，已跳过自动返修。",
      { skipped: true, reviewFailed: true },
    );
    callbacks.onThinking?.(
      formatStageThinking(
        "阶段5：已跳过自动返修",
    ...
    revised = true;
    callbacks.onCheckpoint?.(
      createCheckpoint("after_revision", {
        taskBrief,
        draftContent,
        reviewResults,
        currentContent: revisedContent,
  ...
  if (revised && shouldRunPostRevisionReview) {
    ...
    try {
      ...
      errorChapterWorkflowStep(callbacks, postRevisionWorkflowStep, err);
      callbacks.onThinking?.(
        formatStageThinking(
          "阶段5.5：返修后复审",
    ...
    try {
      ...
      if (repairItems.length > 0) {
        ...
        if (!validation.accepted) {
          errorChapterWorkflowStep(callbacks, repairStep, new Error(validation.reason));
        } else {
          const repairedCandidate = repairedContent.trim();
          let recheckReport: ChapterExecutionReport;
          try {
            recheckReport = await runChapterWorkflowStep(
              callbacks,
              {
          ...
          const recheckPassed =
            extractExecutionRepairItems(recheckReport).length === 0 &&
            recheckReport.status !== "fail" &&
            recheckReport.status !== "unknown";
          executionReportSummary = executionReportToToolSummary(recheckReport, recheckPassed);
          emitDeepChapterActivity(callbacks, {
          ...
          if (recheckPassed) {
            completeChapterWorkflowStep(
              callbacks,
              repairStep,
              `执行失败项返修完成，正文约 ${countChapterChars(repairedCandidate)} 字。`,
              { chars: countChapterChars(repairedCandidate), changed: true },
            );
            finalContent = repairedCandidate;
            revised = true;
            callbacks.onFinalContent?.(finalContent);
          } else {
            errorChapterWorkflowStep(
              callbacks,
              repairStep,
              new Error("返修后执行清单复检仍未通过"),
            );
          }
        }
      }
      ...
      if (shouldRepairPlanDeviation) {
        ...
        try {
          ...
          if (validation.accepted) {
            const repairedCandidate = repairedContent.trim();
            const recheckStep = {
              name: "chapter_plan_deviation_recheck",
              title: "复检计划履约度",
              detail: "返修后再次检查计划偏离点是否消除。",
              params: workflowBaseParams,
            };
            let recheckResult = "";
            try {
            ...
            const parsedRecheck = parseChapterPlanComplianceResult(recheckResult);
            const recheckPassed =
              parsedRecheck.status === "compliant" ||
              parsedRecheck.status === "mostly_compliant";
            emitDeepChapterActivity(callbacks, {
              id: `deep_chapter:plan_deviation_recheck:output:${Date.now()}`,
              stageId: "plan_deviation_recheck",
              kind: "stage_output",
              title: "计划履约复检",
              content: formatPlanComplianceActivityContent(
                parsedRecheck,
                shouldRepairChapterPlanDeviation(parsedRecheck),
              ),
            });
            if (recheckPassed) {
              completeChapterWorkflowStep(
                callbacks,
                repairStep,
                `计划偏离点返修完成，正文约 ${countChapterChars(repairedCandidate)} 字。`,
                { chars: countChapterChars(repairedCandidate), changed: true },
              );
              finalContent = repairedCandidate;
              revised = true;
              ...
              const reason = "返修后计划履约复检仍未通过";
              errorChapterWorkflowStep(callbacks, repairStep, new Error(reason));
              planCompliance = `${reason}：${recheckResult || "未返回有效结果"}`;
              emitDeepChapterActivity(callbacks, {
                id: `deep_chapter:plan_deviation_repair:output:${Date.now()}`,
                stageId: "plan_deviation_repair",
                kind: "stage_output",
                title: "计划偏离点返修",
                content: `${reason}，已保留原正文。`,
              });
            }
          } else {
            errorChapterWorkflowStep(callbacks, repairStep, new Error(validation.reason));
...
function validateChapterPlanRepairResult(
  ...
  if (
    normalizeContentForAnchorCheck(originalContent) ===
    normalizeContentForAnchorCheck(repaired)
  ) {
    return { accepted: false, reason: "返修正文没有发生有效变化" };
  }
...
async function safeBuildChapterContextPack(
  ...
  try {
    ...
    return await waitForAbortable(
      deps.buildContextPack(projectPath, userRequest, chapterNumber),
      signal,
    );
  } catch (error) {
    rethrowIfUserAbort(error, signal);
...
function waitForAbortable<T>(operation: Promise<T>, signal?: AbortSignal): Promise<T> {
  ...
  return new Promise<T>((resolve, reject) => {
    const handleAbort = () => reject(new Error(USER_ABORT_MESSAGE));
