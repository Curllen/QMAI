...
import { loadCharacterStates, type CharacterStateStore } from "@/lib/novel/character-state"
import { loadForeshadowingTracker, type ForeshadowingStore } from "@/lib/novel/foreshadowing-tracker"
import { listSnapshots, loadSnapshot, type ChapterSnapshot } from "@/lib/novel/chapter-ingest"

export type DeviationType = "cognition" | "state" | "continuity" | "foreshadowing"
...
export async function loadReviewEvidence(projectPath: string): Promise<ReviewEvidence> {
  ...
  let characterStates: CharacterStateStore = { characters: [], lastUpdated: "" }
  ...
  try { cognition = await loadCognitionState(projectPath) } catch { rawLoadError = true }
  try { characterStates = await loadCharacterStates(projectPath) } catch { rawLoadError = true }
  try { foreshadowing = await loadForeshadowingTracker(projectPath) } catch { rawLoadError = true }
  try {
    const numbers = await listSnapshots(projectPath)
    const valid = numbers.filter((n) => n > 0)
    if (valid.length > 0) previousSnapshot = await loadSnapshot(projectPath, Math.max(...valid))
  } catch { rawLoadError = true }

  return { cognition, characterStates, foreshadowing, previousSnapshot, internalConflict: false, rawLoadError }
}
